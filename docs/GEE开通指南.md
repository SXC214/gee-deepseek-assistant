...
## 13. （可选）发布应用以告别 7 天重授权

如果你不想每周重新授权一次，可以把 OAuth 应用从 Testing 发布为 Production（见下文常见问题 Q6 的根因说明）：

1. 在 Google Cloud 控制台 → Google Auth Platform → **Audience** 页（即步骤 11.2 添加测试用户的同一页面），点击 **Publish app** 并确认发布。发布可随时进行，Testing 阶段已添加的测试用户不受影响。
2. 发布后回扩展**重新授权一次**，获得长期有效的刷新令牌，此后不再每周过期。

![Audience 页（与步骤 11.2 同一页面），Publish app 入口位于页面顶部](images/oauth-16-add-test-users.png)

> 💡 自用 / 少数人使用场景，发布后**无需做 Google 验证**；授权页若出现「此应用未验证」，通过「高级 → 前往（不安全）」即可继续（详见 Q6）。

**面向公众分发插件时的进阶要求**（仅在「未来公开发布」时才需要，当前自用可跳过）：

- OAuth 同意屏幕需完成**品牌验证 + 敏感范围验证**（免费，通常 ≤10 天），所需材料：公开主页、与主页同域的隐私政策（含 Google 数据使用披露与 Limited Use 声明）、英文演示视频、Search Console 域名验证。
- 按调研判定，earthengine 相关 scope 属于 **sensitive（敏感）级**，非 restricted 级，无需付费安全评估；最终以控制台 **Data Access** 页的实际标注为准。
...
**Q6：授权过一段时间后失效了（约 7 天就要重新授权）？**

根因：OAuth 同意屏幕处于 **Testing（测试）发布状态**时，Google 会对刷新令牌（refresh token）强制 **7 天过期**（Google 官方文档口径），与客户端类型无关——这正是 README 中记录的「令牌约 7 天过期」现象的来源。

解法：在 Google Cloud 控制台 → Google Auth Platform → **Audience** 页点击 **Publish app**（发布为 Production）后，刷新令牌通常长期有效，仅两种例外：用户主动撤销授权、连续约 6 个月未使用。发布后需在扩展内重新授权一次以获得长期令牌（操作步骤见第二部分步骤 13）。

说明：自用 / 少数人场景发布后**无需做 Google 验证**——授权页若出现「此应用未验证」，通过「高级 → 前往（不安全）」即可继续（官方对个人使用有豁免）；但这种方式对不认识的陌生用户体验差，且有用户数上限，公开发布需按第二部分步骤 13 末尾的进阶要求完成验证。
