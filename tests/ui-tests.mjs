...
assert.match(html, /id="conversationTimeline"[^>]*role="log"/);
assert.match(html, /id="liveWorkspace"[^>]*aria-label="实时计划与执行状态"/);
assert.match(html, /id="reasoningStreamSlot"/);
assert.match(html, /id="historyLoadOlderButton"/);
assert.match(html, /id="clearChatButton"/);
assert.match(html, /id="conversationEmpty"/);
assert.match(html, /id="reasoningControl"/);
assert.match(html, /id="reasoningControlPass"/);
...
assert.ok(html.indexOf('id="liveWorkspace"') < html.indexOf('id="planPanel"'), "plan panel must live in the fixed workspace");
assert.ok(html.indexOf('id="liveWorkspace"') < html.indexOf('id="toolResults"'), "tool results must live in the fixed workspace");
assert.ok(html.indexOf('id="conversationTimeline"') < html.indexOf('id="liveWorkspace"'), "history must precede the fixed workspace");
...
assert.match(orchestratorScript, /recordSourceSnapshot\(\{ sources: combinedSources, warnings: combinedWarnings \}\)/);
...
assert.match(script, /function renderReasoningControl\(session\)/);
assert.match(script, /Core \$\{session\.core\?\.length \|\| 0\}/);
assert.match(script, /elements\.reasoningFailureList\.replaceChildren\(\)/);
assert.doesNotMatch(script, /elements\.conversation\.appendChild\(elements\.(?:reasoningControl|toolResults|planPanel|candidatePanel)\)/);
assert.match(styles, /\.reasoning-control\s*\{/);
...
assert.ok(html.indexOf('id="planPanel"') < html.indexOf('id="candidatePanel"'), "candidate must keep a stable workspace position after the plan");
assert.doesNotMatch(script, /function placeCandidateCard\(/);
