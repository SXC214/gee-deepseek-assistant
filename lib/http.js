...
async function singleAttempt(url, fetchOptions, externalSignal, timeoutMs) {
 ...
 * `semaphore`. Retries only happen when `retryable: true`, and only for
 * 429/502/503/504 or network errors; an aborted external signal is never
 * retried. Returns the final Response; callers still inspect `response.ok`.
 */
export async function fetchWithPolicy(url, options = {}) {
  ...
  const execute = async () => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      const outcome = await singleAttempt(url, fetchOptions, signal, timeoutMs);

      if (outcome.response) {
        if (RETRYABLE_STATUSES.has(outcome.response.status) && attempt < maxAttempts - 1) {
          lastOutcome = outcome;
