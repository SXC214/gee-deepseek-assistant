 ...
 * `semaphore`, `consume`. Retries only happen when `retryable: true`, and
 * only for 429/502/503/504 or network errors; an aborted external signal is
 * never retried. When `consume` is provided it reads the body inside the
 * request's timeout/abort umbrella and its return value is resolved instead
 * of the raw Response (retryable statuses are never consumed before a
 * retry). Without `consume`, the final Response is returned and callers still
 * inspect `response.ok`.
 */
