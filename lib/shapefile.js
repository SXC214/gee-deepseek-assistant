...
function parseRecordGeometry(view, contentOffset, contentLength) {
  ...
  if (shapeType === 8 || shapeType === 18) {
    ...
    // Full body: bbox (32) + numPoints (4) after the type word, then 16
    // bytes per point; contentLength is already in bytes here.
    if (contentLength < 40 + 16 * numPoints) {
      throw new Error("MultiPoint 记录长度不足，SHP 文件已损坏");
    }
    const points = [];
  ...
  if (shapeType === 3 || shapeType === 13 || shapeType === 5 || shapeType === 15) {
    ...
    // Full body: header (44) + 4 bytes per part index + 16 bytes per point.
    if (contentLength < 44 + 4 * numParts + 16 * numPoints) {
      throw new Error("PolyLine/Polygon 记录长度不足，SHP 文件已损坏");
    }
