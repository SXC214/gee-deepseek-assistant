 ...
 *
 * The API requires exactly one TableSource URI (400 otherwise): the .shp
 * file itself. Sidecar files (.shx/.dbf/.prj/…) are discovered server-side
 * from the staging bucket, so callers must pass only the .shp gs:// URI.
 */
export async function importTable(token, project, { assetName, uris, charset = "UTF-8" } = {}, { signal } = {}) {
  ...
  if (sourceUris.length !== 1) {
    throw createGeeRestError("上传来源必须恰好为一个 gs:// 引用（.shp 文件本身）", "INVALID_ARGUMENT");
  }
  const body = {
...
export async function pollOperation(token, operationName, {
  ...
  for (;;) {
    ...
    if (operation.error) {
      ...
      // Same-name conflicts are not reported as 409 at acceptance time; the
      // LRO finishes FAILED with "Cannot overwrite asset '<name>'." Map that
      // onto ALREADY_EXISTS so the existing rename UX / noLocalFallback
      // path kicks in (the 409 branch in geeRestPost stays for compatibility).
      if (/cannot overwrite asset/i.test(detail)) {
        throw createGeeRestError(`Earth Engine 资产名已存在：${detail}`, "ALREADY_EXISTS", 0);
      }
...
export async function listAssets(token, project, pageToken = "", { signal } = {}) {
  // The v1 assets endpoint rejects the pageSize query parameter (400
  // "Unknown name \"pageSize\""), so pagination rides pageToken only and
  // the server picks the page size.
  const params = new URLSearchParams();
  if (pageToken) params.set("pageToken", String(pageToken));
  const query = params.toString();
  const path = `/projects/${encodeURIComponent(String(project))}/assets${query ? `?${query}` : ""}`;
