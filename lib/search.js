...
const QUERY_ALIASES = [
  ["ndvi", "vegetation index landsat modis sentinel-2"],
  ["哨兵", "sentinel"],
  ["陆地卫星", "landsat"],
  ["中分辨率成像光谱仪", "modis"],
...
/**
 * Keeps broad searches from returning several near-identical products from a
 * single sensor family. NDVI planning needs at least one official snapshot for
 * Landsat, MODIS and Sentinel-2 so the model can compare temporal coverage.
 */
export function selectDatasetCandidates(entries, query, limit = 6) {
  const ranked = rankEntries(entries, query, Math.max(limit, Math.min(entries.length, 100)));
  const selected = [];
  if (/\bndvi\b/i.test(String(query || ""))) {
    const matchers = [
      (entry) => includesAll(datasetSearchText(entry), ["landsat", "ndvi"]),
      (entry) => includesAll(datasetSearchText(entry), ["modis"])
        && /\bmod13|vegetation indices|\bndvi\b/.test(datasetSearchText(entry)),
      (entry) => /sentinel-2|copernicus_s2/.test(datasetSearchText(entry))
        && /surface reflectance|s2_sr/.test(datasetSearchText(entry))
    ];
    for (const matcher of matchers) {
      const match = ranked.find((entry) => matcher(entry));
  ...
  for (const entry of ranked) {
    ...
    if (isCategoryEntry(entry)) continue;
    if (!selected.some((current) => current.url === entry.url)) selected.push(entry);
  }
...
function datasetSearchText(entry) {
  return normalize(`${entry?.title || ""} ${entry?.slug || ""} ${entry?.searchText || ""}`);
}
...
function isCategoryEntry(entry) {
  ...
  return ["landsat", "modis", "sentinel"].includes(slug);
}
