...
// Word-boundary-anchored context match: a snapshot merely mentioning "info"
// must not wash ee.Info clean, and a longer symbol (ee.Information) must not
// verify a shorter prefix (ee.Info).
function contextVerifies(context, symbols) {
  return symbols.some((symbol) => new RegExp(`\\b${symbol.replaceAll(".", "\\.")}\\b`).test(context));
}
...
export function validateEeCalls(code, { index, verifiedContext = "" } = {}) {
  ...
  for (const match of stripped.matchAll(EE_SYMBOL_PATTERN)) {
    ...
    if (segments.length === 1) {
      ...
      if (contextVerifies(context, [`ee.${className}`])) {
    ...
    for (const methodName of chain) {
      ...
      if (contextVerifies(context, [symbol, `${className}.${methodName}`])) {
...
function stripCommentsAndStrings(code) {
  ...
  while (index < source.length) {
    ...
    if (char === "/" && next === "/") {
      ...
      result += " ";
      index += 1;
      // Only backtick templates may span lines (mirrors the stripper in
      // lib/multi-agent.js); single/double-quoted strings keep the
      // newline-truncation tolerance so an unterminated quote cannot swallow
      // the rest of the script.
      const multiline = char === "`";
      while (index < source.length && source[index] !== char && (multiline || source[index] !== "\n")) {
