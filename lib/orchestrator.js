 ...
 */
import { isLocalBridgeBaseUrl, isOfficialDeepSeekV4Flash } from "./api.js";
...
export function createOrchestrator(deps) {
  ...
  // When the provider never reports usage, the token budget gate silently
  // falls back to the call-count and deadline limits; disclose that state.
  function usageMissingNote(budget) {
    return budget.used() === 0 ? "（usage 缺失，预算闸依赖调用次数与时限兜底）" : "";
  }
  ...
  async function requireSettings() {
    ...
    if (!currentSettings.hasApiKey && !isLocalBridgeBaseUrl(currentSettings.baseUrl)) {
  ...
  async function runPlanTurn(prompt) {
    ...
    try {
      ...
      // Disclosure of non-blocking validation findings (e.g. the static
      // spatial-dataset exemption notice) so they never stay invisible.
      if (activePlan.planWarnings?.length) {
        deps.appendMessage(
          "assistant",
          `校验提示：${activePlan.planWarnings.join("；")}`,
          { purpose: "plan_action" }
        );
      }
      planStage = "保存计划版本";
  ...
  async function runMultiAgentGeneration(settings, state, confirmedRevision) {
    ...
    try {
      return await runMultiAgentPipeline(settings, state, confirmedRevision, budget);
    } catch (error) {
      if (isAbortError(error)) throw error;
      deps.appendMessage(
        "assistant",
        `多 Agent 流水线未能完成：${safeError(error)}${usageMissingNote(budget)}。已回退到单轮生成。`,
        { purpose: "plan_action" }
      );
    }
    const messages = buildGenerationMessages(activePlan, state, settings);
  ...
  async function runMultiAgentPipeline(settings, state, confirmedRevision, budget) {
    ...
    deps.appendMessage(
      "assistant",
      `多 Agent 流水线完成 · ${ledger.calls()} 次模型调用 · 累计 ${budget.used()} tokens。${usageMissingNote(budget)}`,
      { purpose: "plan_action" }
    );
