...
import { sanitizeReasoningSession } from "./reasoning-state.js";
...
export const REASONING_SESSION_KEY = "reasoningSessionV1";
...
/** Reads the durable control ledger; private reasoning is never part of it. */
export async function readReasoningSession(storage) {
  const stored = await storage.get(REASONING_SESSION_KEY);
  return sanitizeReasoningSession(stored?.[REASONING_SESSION_KEY]);
}

/** Writes a sanitized ledger or removes the key when the session is absent. */
export async function writeReasoningSession(storage, session) {
  const sanitized = sanitizeReasoningSession(session);
  if (!sanitized) {
    await storage.remove(REASONING_SESSION_KEY);
    return null;
  }
  await storage.set({ [REASONING_SESSION_KEY]: sanitized });
  return sanitized;
}
