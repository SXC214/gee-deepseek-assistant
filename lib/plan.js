 ...
 */
export function createPlanSession(originalRequest) {
...
export function sanitizePlanSession(value) {
  ...
  const readyValidation = candidatePlan?.status === "ready"
    ? validatePlanResponse(candidatePlan, sources, { originalRequest })
    : null;
...
function normalizePlan(plan) {
  ...
  return {
    ...
    questions: asArray(plan.questions).map(cleanText).filter(Boolean)
  };
}
