...
/**
 * Builds a deterministic, non-confirmable TODO plan when a model cannot
 * produce a structurally valid todo_v1 response after one repair attempt.
 * The skeleton deliberately contains no model-supplied dataset claims or
 * analysis decisions; the next user turn can safely resume normal planning.
 */
export function createSafeTodoFallbackPlan(originalRequest) {
  const goal = cleanText(originalRequest) || "完成当前 Google Earth Engine 分析需求";
  return {
    ...
    todoList: [
      {
        id: "todo_1",
        title: "确认分析目标与边界",
        objective: "核对指标、研究区、时间范围、统计口径和输出要求",
        evidenceNeeded: ["用户原始需求与后续确认"],
        status: "in_progress",
        result: ""
      },
      {
        id: "todo_2",
        title: "核验官方数据与文档",
        objective: "从 Earth Engine 官方来源核验候选数据集及关键限制",
        evidenceNeeded: ["Earth Engine Data Catalog 或官方 API 文档"],
        status: "pending",
        result: ""
      },
      {
        id: "todo_3",
        title: "设计分析与质量控制",
        objective: "确定处理步骤、质量控制、时间聚合和空间统计方法",
        evidenceNeeded: ["已确认需求与已核验官方资料"],
        status: "pending",
        result: ""
      },
      {
        id: "todo_4",
        title: "形成可审阅方案",
        objective: "汇总方法、输出、假设和风险，等待用户确认",
        evidenceNeeded: ["前序 TODO 结果与用户确认"],
        status: "pending",
        result: ""
      }
    ],
    ...
    nextTodoId: "todo_2",
    goal,
    researchSummary: "模型计划结构未通过校验，已切换为本地安全任务骨架；尚未形成数据或方法结论。",
    datasets: [],
    requirements: {
      area: "",
      timeRange: "",
    ...
    outputs: [],
    assumptions: [],
    risks: ["当前仅为恢复规划流程的任务骨架，所有数据与方法结论仍需核验。"],
    revisionSummary: ["自动结构修复未通过后创建本地 4 项 TODO 骨架。"],
    questions: [{
      id: "fallback_plan_flow",
      prompt: "模型未能生成合规的任务清单，是否按这套本地 4 项默认流程继续规划？",
      options: [
        {
          id: "continue_default_flow",
          label: "按默认流程继续",
          description: "继续核验需求、官方数据、分析方法和输出。",
          recommended: true
        },
        {
          id: "revise_request_scope",
          label: "先调整需求",
          description: "先补充或修改研究区、时间、指标、统计口径或输出。",
          recommended: false
        }
      ],
      allowFreeText: true
...
/**
 * Repairs mechanical todo_v1 invariants without asking the model to rewrite
 * factual plan content. The returned object always carries the complete
 * top-level shape expected by validateTodoWorkflow; only workflow bookkeeping
 * (task count/ids/status pointers/phase and option ids) is synthesized.
 */
export function canonicalizeTodoPlanStructure(plan, { originalRequest = "" } = {}) {
  const normalized = normalizePlan(isPlainObject(plan) ? plan : {});
  const fallback = createSafeTodoFallbackPlan(originalRequest || normalized.goal);
  const corrections = [];
  const sourceTasks = normalized.todoList.slice(0, 8);
  if (normalized.todoList.length > 8) corrections.push("TODO 超过 8 项，已保留前 8 项");

  while (sourceTasks.length < 4) {
    const template = fallback.todoList[sourceTasks.length];
    sourceTasks.push({ ...template, evidenceNeeded: [...template.evidenceNeeded] });
  }
  if (normalized.todoList.length < 4) corrections.push("TODO 不足 4 项，已补齐本地安全任务");

  const usedIds = new Set();
  const todoList = sourceTasks.map((todo, index) => {
    const template = fallback.todoList[Math.min(index, fallback.todoList.length - 1)];
    let id = cleanText(todo.id) || `todo_${index + 1}`;
    if (usedIds.has(id)) {
      id = uniqueTodoId(usedIds, index + 1);
      corrections.push("重复 TODO id 已重新编号");
    }
    usedIds.add(id);
    const result = cleanText(todo.result);
    let status = TODO_STATUSES.has(todo.status) ? todo.status : "pending";
    if (status === "completed" && !result) {
      status = "pending";
      corrections.push("无结果的 completed TODO 已恢复为 pending");
    }
    return {
      id,
      title: cleanText(todo.title) || template.title,
      objective: cleanText(todo.objective) || template.objective,
      evidenceNeeded: asArray(todo.evidenceNeeded).map(cleanText).filter(Boolean).length
        ? asArray(todo.evidenceNeeded).map(cleanText).filter(Boolean)
  ...
  const completedThisTurn = normalized.completedThisTurn.filter((id) => (
    todoList.some((todo) => todo.id === id && todo.status === "completed")
  ));
  if (completedThisTurn.length !== normalized.completedThisTurn.length) {
    corrections.push("completedThisTurn 已移除无效引用");
  }

  const structurallyReady = normalized.status === "ready"
    && todoList.every((todo) => todo.status === "completed" && todo.result)
    && normalized.questions.length === 0;
  const questions = structurallyReady ? [] : canonicalizeTodoQuestions(normalized.questions);

  let currentTodo = null;
  if (!structurallyReady) {
    currentTodo = todoList.find((todo) => todo.status === "in_progress")
      || todoList.find((todo) => todo.status !== "completed")
      || null;
    for (const todo of todoList) {
      if (todo === currentTodo) todo.status = "in_progress";
      else if (todo.status === "in_progress") todo.status = "pending";
    }
  }
  const pendingTasks = todoList.filter((todo) => todo.status === "pending");
  const nextTodo = currentTodo
    ? pendingTasks.find((todo) => todoList.indexOf(todo) > todoList.indexOf(currentTodo)) || pendingTasks[0] || null
    : null;
  if (normalized.currentTodoId !== (currentTodo?.id || "") || normalized.nextTodoId !== (nextTodo?.id || "")) {
    corrections.push("currentTodoId/nextTodoId 已按任务状态重建");
  }

  const canonical = {
    status: structurallyReady ? "ready" : "needs_clarification",
    phase: structurallyReady ? "reviewing" : "clarifying",
    todoList,
    completedThisTurn,
    currentTodoId: currentTodo?.id || "",
    nextTodoId: nextTodo?.id || "",
    goal: normalized.goal || cleanText(originalRequest) || fallback.goal,
    researchSummary: normalized.researchSummary,
    ...
    revisionSummary: mergeRevisionSummary(normalized.revisionSummary, corrections),
    questions
  };
  return { plan: canonical, corrections: [...new Set(corrections)] };
}

function canonicalizeTodoQuestions(value) {
  const questions = asArray(value).map(normalizeQuestion).filter((question) => question.prompt).slice(0, 3);
  if (!questions.length) {
    return [{
      id: "confirm_canonical_plan",
      prompt: "任务清单已完成结构校正，是否按当前计划继续调研？",
      options: [
        { id: "continue_plan", label: "按当前计划继续", description: "继续核验数据、方法和输出。", recommended: true },
        { id: "revise_plan", label: "先调整计划", description: "先补充或修改分析范围与口径。", recommended: false }
      ],
      allowFreeText: true
    }];
  }
  return questions.map((question, questionIndex) => {
    const options = question.options.slice(0, 3).map((option, optionIndex) => ({
      ...option,
      id: cleanText(option.id) || `question_${questionIndex + 1}_option_${optionIndex + 1}`
    }));
    while (options.length < 2) {
      const index = options.length + 1;
      options.push({
        id: `question_${questionIndex + 1}_option_${index}`,
        label: index === 1 ? "采用推荐方案" : "调整后继续",
        description: index === 1 ? "按当前推荐继续。" : "先补充自定义要求。",
        recommended: index === 1
      });
    }
    const usedOptionIds = new Set();
    for (const [index, option] of options.entries()) {
      if (usedOptionIds.has(option.id)) option.id = `question_${questionIndex + 1}_option_${index + 1}`;
...
function uniqueTodoId(usedIds, start) {
  let index = start;
  while (usedIds.has(`todo_${index}`)) index += 1;
  return `todo_${index}`;
}

function mergeRevisionSummary(value, corrections) {
  const summary = asArray(value).map(cleanText).filter(Boolean);
  if (!corrections.length) return summary;
  return [...summary, `本地结构校正：${[...new Set(corrections)].join("；")}。`];
}
