...
export function applyPlanResponse(session, response) {
  ...
  return {
    ...
    planStale: false,
    // Transient disclosure channel: sanitizePlanSession drops this field on
    // restore, so it never becomes part of the persisted schema.
...
function hasStaticCoverageSignal(coverageText) {
  // "2000-present / 2000-至今" describes an in-service time series, never a
  // static snapshot: detect the open end first so such datasets stay gated
  // by temporal coverage checks.
  if (OPEN_ENDED_COVERAGE_PATTERN.test(coverageText)) return false;
...
const OPEN_ENDED_COVERAGE_PATTERN = /\bpresent\b|\bnow\b|\bongoing\b|至今|目前|现在|持续/i;
...
// counts as a static signal when it is tied to a spatial-extent role. Bare
// "作为…掩膜" stays out on purpose: a cloud-mask QA dataset is temporal.
const SPATIAL_DATASET_IDENTITY_PATTERN = /boundar|borders?|administrative|geometr|边界|行政|区划|矢量/i;
const SPATIAL_EXTENT_ROLE_PATTERN = /空间范围|研究区(?:域|边界)?|区域(?:边界|范围|裁剪)|作为.{0,12}(?:边界|范围)|边界掩膜|用于(?:界定|限定|裁剪|约束)|study area|spatial extent|region of interest/i;
