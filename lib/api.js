 ...
 * empty API Key; every other endpoint still requires one. Shared by the
 * orchestrator settings gate; service-worker.js keeps an identical local
 * copy (pinned by the UI consistency tests), so both must stay in sync.
 */
export function isLocalBridgeBaseUrl(baseUrl) {
  try {
