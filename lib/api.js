...
const DEEPSEEK_V4_MODEL_PREFIXES = ["deepseek-v4-flash", "deepseek-v4-pro"];
const DEEPSEEK_V4_FLASH_PREFIX = "deepseek-v4-flash";
...
function isDeepSeekV4ModelName(model) {
  ...
  return DEEPSEEK_V4_MODEL_PREFIXES.some((prefix) => name.startsWith(prefix));
}
...
export function isOfficialDeepSeekV4Flash(baseUrl, model) {
  return isOfficialDeepSeekV4(baseUrl, model)
    && normalizedModelName(model).startsWith(DEEPSEEK_V4_FLASH_PREFIX);
}
