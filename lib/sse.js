  ...
  const processFrame = (frame) => {
    ...
    for (const choice of Array.isArray(payload?.choices) ? payload.choices : []) {
      ...
      if (Array.isArray(delta.tool_calls) && delta.tool_calls.length) {
        emit({ type: "TOOL_CALL_DELTA", toolCalls: delta.tool_calls });
      }
...
/**
 * Races an HTTP error body read against the signal so a stalled body cannot
 * outlive the request: plain `response.text()` ignores signals in mocks and
 * can hang forever, while the caller's timeout/manual stop must always win.
 */
function abortAwareErrorBody(bodyPromise, signal) {
  const pending = Promise.resolve(bodyPromise ?? "");
...
export async function consumeSseResponse(response, { onEvent, signal } = {}) {
  if (!response?.ok) {
    const raw = await abortAwareErrorBody(response?.text?.(), signal) || "";
    let payload = null;
    try {
