...
| `PROXY_API_KEY` | 空 | Bearer 鉴权密钥。**未设置时跳过鉴权并在 stderr 打印警告**；设置后 `Authorization: Bearer <key>` 必须精确匹配。 |
...
- **PROXY_API_KEY**：建议设置，防止本机其他进程冒用桥接。扩展端在设置中填入相同字符串即可。
...
1. 启动桥接：`node tools/cli-bridge/server.mjs`（建议先设置 `PROXY_API_KEY`）。
2. 打开扩展侧栏 → 设置：
   - 方式一：服务商预设选择 **「本地 Qoder CLI 桥接」（local-qoder）**，模型默认 `qoder-cli`；
   - 方式二：手填 `baseUrl = http://127.0.0.1:3000/v1`，并勾选「兼容端点流式」（OpenAI 兼容 SSE）。
3. 若桥接设置了 `PROXY_API_KEY`，在扩展 API Key 输入框填入相同值；未设置则留空（扩展对 localhost 端点豁免空 Key 校验）。
