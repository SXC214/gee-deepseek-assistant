...
7. 在“模型与项目设置”中填写 API 地址、模型名称和 API Key。

DeepSeek 默认 API 地址为 `https://api.deepseek.com`。扩展会调用其 `/chat/completions` 路径。如果服务商要求完整路径，也可以直接把 API 地址填写为以 `/chat/completions` 结尾的地址。
